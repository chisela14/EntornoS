...
public class UserService implements UserDetailsService {
	...
	public boolean checkExist(User s) {
		...
		User checkUser = rep.findById(s.getUser()).orElse(null);
		// Comprueba que no existe el email en la base de datos
		List<User> checkPassword =
		rep.findByEmail(s.getEmail());
	...
	public void add(User s, String siteURL) throws UnsupportedEncodingException, MessagingException {
		...
		// Genera una cadena aleatoria que guarderemos en el código de verificación 
		// y que le enviaremos al usuario para saber que es él.
